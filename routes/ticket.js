...
const upload = multer({
  storage: multerS3({
    ...
    acl:"public-read"//It is used for accessing the media directly publicly
  }),
});
...
router.get("/", (req, res) => {
  try {
    const sql =
      "SELECT * FROM ticket LEFT JOIN user ON ticket.user=user.user_id LEFT JOIN location ON ticket.location=location.location_id LEFT JOIN customer ON ticket.customer=customer.customer_id";
      ...
      if (err) {
        ...
        if (result.length > 0) {
          ...
          let resultData = result.map((item) => {
            ...
            const upload_pdf_url = urlCreator(item.upload_pdf);
            return {
...
router.get(`/user/:id`, (req, res) => {
  try {
    const sql = `SELECT * FROM ticket LEFT JOIN user ON ticket.user=user.user_id LEFT JOIN location ON ticket.location=location.location_id LEFT JOIN customer ON ticket.customer=customer.customer_id WHERE user='${req.params.id}'`;
    db.query(sql, async (err, result) => {
      if (err) {
        res.status(400).json({ message: err.message });
      } else {
...
router.post(
  ...
  (req, res) => {
    try {
      ...
      const upload_pdf = req.files.upload_pdf[0].key;
      const ticketObj = {
        user: req.body.user,
